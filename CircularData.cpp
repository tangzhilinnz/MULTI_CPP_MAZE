...
CircularData::CircularData()
	: head(CIRCULAR_DATA_SIZE),
	tail(CIRCULAR_DATA_SIZE)
{
}
...
{
	std::lock_guard<std::mutex> lock(mtx);
	return (tail.NextIndex() == head.Index());
}

size_t CircularData::Size() const
{
	std::lock_guard<std::mutex> lock(mtx);
	...
	return (CIRCULAR_DATA_SIZE - head.Index()) + tail.Index();
}
...
bool CircularData::PushBack(const Position& pPos)
{
	...
	if (tail.NextIndex() == head.Index()) {
		return false; // FULL
	}

	data[tail.Index()] = pPos;
	tail++;
