...
public:
	// Needs to be a power of 2
	static const int CIRCULAR_DATA_SIZE = 1 << 13;
	static const int MASK = CIRCULAR_DATA_SIZE - 1;
...
private:
	...
	// CACHE LINE PADDING
	// Aligning these to separate cache lines (64 bytes) prevents false sharing
	// between the producer thread writing 'back' and consumer writing 'front'.
	alignas(64) std::atomic<unsigned int> front;
	alignas(64) std::atomic<unsigned int> back;

	char pad[64]; // Extra safety padding at the end
};
